...
heartbeat(Pool) ->
  shackle:call(Pool, {heartbeat}, ?TIMEOUT).

position(Pool) ->
  shackle:call(Pool, {position}, ?TIMEOUT).
...
handle_request({Operation}, #state {
  request_counter = RequestCounter
} = State) ->
  SeqNo = seq_no(RequestCounter),
  RequestId = arithmetic_protocol:request_id(RequestCounter),
  Data = arithmetic_protocol:request(RequestId, Operation, SeqNo),
